...
const ElvenAssassinLaunch: React.FC = () => {
    ...
    const fetchTimerSettings = async () => {
      try {
        const {
          data,
          error
        } = await supabase.from('settings').select('timer_duration').eq('id', 'global').single();
        if (error) {
  ...
  const createGameSession = async (gameName: string) => {
    try {
      const {
        data: gameData
      } = await supabase.from('games').select('id').eq('title', gameName).single();
      if (!gameData) {
      ...
      } = await supabase.from('game_sessions').select('*').eq('game_id', gameData.id).is('completed', false).limit(1);
        ...
        <div className="glass p-8 rounded-2xl space-y-8 relative overflow-hidden border border-white/20 bg-[#222232]/50 backdrop-blur-xl">
          <div className="text-center">
            ...
            <div className="flex gap-4 justify-center">
              ...
              <span className="inline-block px-4 py-1 rounded-full text-sm text-white/90 bg-indigo-500/30 backdrop-blur-sm border border-indigo-500/30">
                Fantasy
              </span>
